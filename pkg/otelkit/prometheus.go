...
func NewPrometheusServiceMeter(ctx context.Context, conf *PrometheusServiceMeterConfig) *PrometheusServiceMeter {
	...
	// TODO
	errorRequestCounter, err := meter.SyncInt64().Counter("error_request", instrument.WithDescription("count number of error request"))
	if err != nil {
		logger.Fatal("failed to create error request counter")
	}
